...
class ResCountryState(models.Model):
    ...
    old_province_mapping = fields.Text(
        ...
        help='Danh sách các tỉnh thành cũ được sáp nhập vào tỉnh thành này (JSON format)'
    )
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        res = super(ResCountryState, self).name_search(name, args, operator, limit)
        for record in res:
            state = self.browse(record[0])  
            if not state.is_vietnam_new_province and state.country_id.code == 'VN':
                res.remove(record)
        return res
